...
/**
 * Screenshot baselines for the Meal Review screen in both themes. Regenerate with
 * `./gradlew :feature:mealreview:recordRoborazziDebug` and review the PNG diff in the PR.
 */
@RunWith(RobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(sdk = [34], qualifiers = "w411dp-h891dp-420dpi")
class MealReviewScreenScreenshotTest {
    ...
    @Test
    fun meal_review_screen_unrated() = captureLightAndDark("meal_review_screen_unrated") {
        MealReviewScreen(state = sampleMealReviewState.copy(rating = 0, note = ""))
    }

    @Test
    fun meal_review_screen_rated() = captureLightAndDark("meal_review_screen_rated") {
        MealReviewScreen(state = sampleMealReviewState)
    }
}
